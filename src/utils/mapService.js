export default class MapService {
    ...
    //添加标记图标
    addMarker(lngLat, title, callback) {
        this.marker = new AMap.Marker({
            label: {content: title, offset: new AMap.Pixel(-40, -22)},
        });
    ...
    static gpsTransform(obj) {//加上static可以不用实例化使用该方法，详见文档说明
        return new Promise((resolve, reject) => {
            ...
            switch (Object.prototype.toString.call(obj.lnglat)) {
                case '[object String]':
                    para = 'key=' + obj.key + '&locations=' + obj.lnglat + '&coordsys=gps';
                    break;
                case '[object Array]':
                    para = 'key=' + obj.key + '&locations=' + obj.lnglat[0] + ',' + obj.lnglat[1] + '&coordsys=gps';
                    break;
            }
            request.send(para);
            request.onload = () => {
    ...
    static webLnglatToAddress(obj) {
        return new Promise((resolve, reject) => {
            ...
            switch (Object.prototype.toString.call(obj.lnglat)) {
                case '[object String]':
                    para = 'key=' + obj.key + '&location=' + obj.lnglat + '&extensions=all';
                    break;
                case '[object Array]':
                    para = 'key=' + obj.key + '&location=' + obj.lnglat[0] + ',' + obj.lnglat[1] + '&extensions=all';
                    break;
            }
            request.send(para);
            request.onload = () => {
